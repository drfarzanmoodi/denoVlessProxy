...
const handler = async (req: Request): Promise<Response> => {
  ...
  socket.onmessage = async (e) => {
    try {
      ...
      if (remoteConnection) {
        ...
        if (clientID !== userID) {
          console.log('Auth Failed: Invalid UUID');
          return;
        }
        ...
        remoteConnection.readable.pipeTo(new WritableStream({
          start() {
            socket.send(new Blob([new Uint8Array([0, 0])]));
          },
          write(chunk) {
            socket.send(new Blob([chunk]));
          }
        })).catch(() => console.log('Remote Connection Closed'));
      }
    } catch (err) {
  ...
  socket.onclose = () => console.log('VPN Connection Closed');
